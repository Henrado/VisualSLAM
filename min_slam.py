...
class VisualOdometry():
    ...
    def __init__(self, img_dir, calib_dir):
        
        print("Loading Images")
        self.calibration = self._load_calib(os.path.join(calib_dir, 'calib_cam_to_cam.txt'))
        self.K_l = self.calibration["GrayLeft"]["calibration"]
        self.K_r = self.calibration["GrayRight"]["calibration"]
        self.P_l = self.calibration["GrayLeft"]["rectified_projection"]
        self.P_r = self.calibration["GrayRight"]["rectified_projection"]

        self.images_l = self._load_images(os.path.join(img_dir, 'image_00', 'data'))[:200]
        self.images_r = self._load_images(os.path.join(img_dir, 'image_01', 'data'))[:200]
    ...
    def _load_calib(filepath):
        ...
        for camera_id in VisualOdometry.camera_id.values():
            calibration_for_camera[VisualOdometry.camera_from_id[camera_id]] = {
    ...
    def loop_detection(self, descriptors):
        ...
        descs = [cv2.ORB.from_cv_descriptor(desc) for desc in descriptors]
        self.db.add(descs)
...
def main():
    img_dir = 'KITTI_2_FULL/2011_10_03/2011_10_03_drive_0042_extract'  # Try KITTI_sequence_2
    calib_dir = 'KITTI_2_FULL'
    vo = VisualOdometry(img_dir, calib_dir)
    ...
    plotting.visualize_paths(gt_path, estimated_path, "Stereo Visual Odometry",
                             file_out=os.path.basename(calib_dir) + ".html")
